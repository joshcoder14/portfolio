...
class ID_Lemon_Way {
	...
	function init() {
		...
		if (self::$lemonway_active) {
			...
			if (isset($gateway_settings['elw']) && $gateway_settings['elw'] == '1') {
				...
				add_filter('idc_creator_settings', array($this, 'lemonway_creator_settings'));
	...
	public function lemonway_admin_settings() {
		...
		$image_url = plugins_url('/images/lemonway.png', __FILE__);
		?>
	...
	public function lemonway_creators() {
		$gateway_settings = get_option('memberdeck_gateways');
		return (isset($gateway_settings['lemonway_creators']) ? $gateway_settings['lemonway_creators'] : 0);
	}

	/**
	 * Filter function to show payments/creator settings if lemonway is active and creator settings enabled
	 */
	public function lemonway_creator_settings($settings) {
		$lemonway_creators = $this->lemonway_creators();
		$settings['lemonway_creators'] = $lemonway_creators;
		return $settings;
	}
	...
	public function idc_lemonway_to_creator_settings($settings) {
		$settings['lemonway'] = 1;
		return $settings;
	}
			...
			if (isset($_POST['creator_settings_submit'])) {
				...
				if (isset($_POST['lemonway_connect'])) {
					...
					if (empty($connect_settings) || !isset($connect_settings['lemonway_connect'])) {
						...
						$gateway_settings = get_option('memberdeck_gateways');
						require dirname(__file__).'/lib/LemonWayIncludes.php';
