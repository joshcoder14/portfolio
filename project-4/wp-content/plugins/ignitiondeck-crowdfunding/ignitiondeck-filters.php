...
function id_text_filters_n( $translated_text, $single, $plural, $number, $domain ) {
	...
	if ( in_array( $domain, $domain_list ) || strpos( $domain, 'fivehundred' ) !== false ) {
		...
		} elseif ( strpos( $single, 'Day Left' ) !== false ) {
...
function id_funds_raised( $amount ) {
	...
	if ($global_currency == 'BTC' || $global_currency == 'credits') {
		return apply_filters( 'id_number_format', $amount, '8', '.', '' );
	} else {
...
function id_project_goal( $goal, $post_id, $noformat = false ) {
	...
	if ( $noformat ) {
		...
		if ($global_currency == 'BTC' || $global_currency == 'credits') {
			return apply_filters( 'id_display_currency', apply_filters( 'id_number_format', $goal, 8 ), $post_id );
		} else {
