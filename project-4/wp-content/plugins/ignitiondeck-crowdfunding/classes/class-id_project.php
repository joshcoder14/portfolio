...
class ID_Project {
	...
	function __construct( $id = null ) {
		...
		add_action( 'id_modify_order', array( $this, 'flush_project_parent' ), 10, 3 );
	}

	function the_project() {
		if (function_exists('idf_get_object')) {
	...
	function flush_project_orders( $order_id, $action = null, $the_order = null ) {
		...
		if ( ! empty( $the_order->project_id ) && $the_order->status !== 'failed' ) {
			idf_flush_object( 'id_project-get_project_orders-' . $the_order->project_id );
		}
	}
	...
	function get_project_raised( $noformat = false ) {
		...
		if ($global_currency == 'BTC' || $global_currency == 'credits') {
			return ( $noformat ? $raised : apply_filters( 'id_display_currency', apply_filters( 'id_number_format', $raised, 8 ), $this->id ) );
		} else {
	...
	function flush_project_raised( $order_id, $action = null, $the_order = null ) {
		...
		if ( ! empty( $the_order->project_id ) && $the_order->status !== 'failed' ) {
			idf_flush_object( 'id_project-get_project_raised-' . $the_order->project_id );
		}
	}
	...
	function end_date( $post_id = null ) {
		...
		if( is_numeric($end_date) ) $end_date = date( $date_format, $end_date ); //removing the string to time conversion as it already saves the timestamp in database now with the new code added for issues #28
		else $end_date = 'Updating Date Formats in background. <a href="?post_type=ignition_product&force_data_update_id_cron">Click here</> to run now';
		return $end_date;
	}
	...
	function days_left( $end_date = null ) {
		...
		// if date format did not updated show notice
		if( is_bool($end_date) ) return 'Updating Date Formats in background. <a href="?post_type=ignition_product&force_data_update_id_cron">Click here</> to run now';
	...
	public static function insert_project( $args ) {
		...
		$sql       = $wpdb->prepare(
			'INSERT INTO ' . $wpdb->prefix . 'ign_products (
			product_name,
			ign_product_title,
			ign_product_limit,
			product_details,
	...
	// Method to be used by cron to update all dates to timestamp if they aren't
	public static function update_date_to_timestamp( $project_id = null ) {
		if ( empty( $project_id ) ) {
			global $wpdb;
			// looking for posts having old string dates
			$meta_rows = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}postmeta WHERE meta_key IN ('ign_start_date','ign_fund_end') and concat('',meta_value * 1) != meta_value", ARRAY_A );

			// if there are no more posts with string dates, add an option which stops the update cron
			if( count($meta_rows) < 1 ) update_option( 'id_date_data_updated', 'yes' );
			else{
				foreach ($meta_rows as $meta_row) {
					//create date object from wp saved date format
					$dateObj=date_create_from_format(get_option( 'date_format' ) , $meta_row['meta_value']);

					//fallback to generic date function if date object fails
					$new_date = $dateObj ? $dateObj->getTimestamp() : strtotime($meta_row['meta_value']);
					update_post_meta( $meta_row['post_id'], $meta_row['meta_key'], $new_date ); //update with the  new date
				}
			}
		} else {
			// if a specific ID of project is passed, it only updates that particular project. This can be used in future or while debugging
			$project = new ID_Project( $project_id );
			$post_id = $project->get_project_postid();
			$old_start_date = get_post_meta( $post_id, 'ign_start_date', true );
			if( !is_int( $old_start_date ) ){
				//create date object from wp saved date format
				$dateObj=date_create_from_format(get_option( 'date_format' ) , $old_start_date);

				//fallback to generic date function if date object fails
				$new_start_date = $dateObj ? $dateObj->getTimestamp() : strtotime($old_start_date);
				update_post_meta( $post_id, 'ign_start_date', $new_start_date ); //update with the  new date
			}
			$old_end_date = get_post_meta( $post_id, 'ign_fund_end', true );
			if( !is_int( $old_end_date ) ){
				//create date object from wp saved date format
				$dateObj=date_create_from_format(get_option( 'date_format' ) , $old_end_date);

				//fallback to generic date function if date object fails
				$new_end_date = $dateObj ? $dateObj->getTimestamp() : strtotime($old_end_date);
				update_post_meta( $post_id, 'ign_fund_end', $new_end_date ); //update with the  new date
			}
		}
	}
	...
	function get_project_raised_by_dates( $start_date, $end_date ) {
		...
		date_default_timezone_set( $tz );
		$sql = $wpdb->prepare(
			'SELECT SUM(prod_price) AS raise FROM ' . $wpdb->prefix . 'ign_pay_info WHERE product_id = %d
	...
	function get_project_orders_by_dates( $start_date, $end_date ) {
		...
		date_default_timezone_set( $tz );
		$sql = $wpdb->prepare(
			'SELECT COUNT(*) AS count FROM ' . $wpdb->prefix . 'ign_pay_info WHERE product_id = %d' .
				' AND (created_at >= %s AND created_at <= %s)',
			$this->id,
			$start_date . ' 00:00:00',
			$end_date . ' 23:59:59'
	...
	function flush_project_parent( $order_id, $action = null, $the_order = null ) {
		...
		idf_flush_object( 'id_project-get_project_raised-' . $parent_project_id );
		idf_flush_object( 'id_project-get_project_orders-' . $parent_project_id );
	}
}
