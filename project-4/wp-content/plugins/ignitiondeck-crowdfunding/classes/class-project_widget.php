		...
		'fields'      => array(
			...
			array(
				...
				'options' => array(
					...
					array(
						'name'  => 'Mini Deck on all Pages except Project Page',
						'value' => 'miniallpages',
					), //translate
				),
			),
		...
		if ( isset( $post ) ) {
			...
			if ( isset( $params['widget_options'] ) ) {
				if ( $params['widget_options'] == 'miniwidget' ) {
					$show_mini = true;
				} elseif ( $params['widget_options'] == 'fullwidget' ) {
					$show_mini = false;
				} elseif ( $params['widget_options'] == 'miniallpages' && $page_project_id > 0 ) { //2nd condition means it is a project page, containing some non-zero project_id
					$show_mini = false;
				} else {
					$show_mini = true;
				}
