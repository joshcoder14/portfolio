...
function id_projectFAQ( $attrs ) {
	...
	if ( isset( $attrs['product'] ) ) {
		...
		if ( ! empty( $faq_output ) ) {
			$content .= '<div class="ignitiondeck"><h3 class="product-dashed-heading">' . __( 'Project FAQ', 'ignitiondeck' ) . '</h3>';
			$content .= '<div id="prodfaq">';
			$content .= $faq_output;
			$content .= '<div>' . apply_filters( 'id_faqs', '', $project_id ) . '</div>';
			$content .= '</div></div>';
		} elseif ( has_filter( 'id_faqs' ) ) {
			$content .= '<div class="ignitiondeck"><h3 class="product-dashed-heading">' . __( 'Project Faq', 'ignitiondeck' ) . '</h3>';
			$content .= '<div id="prodfaq">';
			$content .= apply_filters( 'id_faqs', '', $attrs );
			$content .= '</div></div>';
		}
	}
	return $content;
...
function id_projectUpdates( $attrs ) {
	...
	if ( isset( $attrs['product'] ) ) {
		...
		if ( ! empty( $update_data ) ) {
			$content .= '<div class="ignitiondeck"><h3 class="product-dashed-heading1">' . apply_filters( 'idcf_updates_text', __( 'Updates', 'ignitiondeck' ) ) . '</h3>';
			$content .= '<div id="produpdates">';
			$content .= $update_data;
			$content .= '<div>' . apply_filters( 'id_updates', '', $attrs ) . '</div>';
			$content .= '</div></div>';
		} elseif ( has_action( 'id_updates' ) ) {
			$content .= '<div class="ignitiondeck"><h3 class="product-dashed-heading1">' . apply_filters( 'idcf_updates_text', __( 'Updates', 'ignitiondeck' ) ) . '</h3>';
			$content .= '<div id="produpdates">';
			$content .= apply_filters( 'id_updates', '', $attrs );
			$content .= '</div></div>';
		}
	}
	return $content;
...
function id_projectPageContent( $attrs ) {
	if ( !isset($_GET['purchaseform']) && isset( $attrs['product'] ) ) {
		ob_start();
		$project_id  = $attrs['product'];
		$project     = new ID_Project( $project_id );
		$the_project = $project->the_project();
...
function id_projectGrid( $attrs ) {
	...
	if ( isset( $attrs['orderby'] ) ) {
		...
		} elseif ( $attrs['orderby'] == 'percent_raised' ) {
			$args['orderby']  = 'meta_value_num';
			$args['meta_key'] = 'ign_percent_raised';
		} elseif ( $attrs['orderby'] == 'funds_raised' ) {
			$args['orderby']  = 'meta_value_num';
