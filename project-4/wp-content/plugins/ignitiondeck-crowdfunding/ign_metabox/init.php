...
function idcf_metabox_scripts() {
	...
	if ( ! empty( $post ) ) {
		if ( $post->post_type == 'ignition_product' ) {
			...
			wp_add_inline_script( 'idcf-metabox', "var memberdeck_global_currency  = '". ( ! empty( $global_currency ) ? $global_currency : 'USD' ) . "';" );
...
class ign_cmb_Meta_Box {
	...
	function __construct( $meta_box ) {
		...
		add_action( 'admin_init', array( $this, 'save' ), 3, 2 );
	}
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'text_medium':
					...
					break;
				case 'text_date':
					echo '<input class="cmb_text_small cmb_datepicker" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', (isset( $meta ) && is_numeric($meta) ) ? date(get_option( 'date_format' ), $meta) : $field['std'], '" />'; //converting timestamp custom meta for text based dates to make it consistent with datepicker used. Applies on "Edit post" screen
					break;
				case 'text_money':
					if ($global_currency == 'BTC' || $global_currency == 'credits') {
						echo '<input class="cmb_text_money" type="text" name="', $field['id'], '" id="', $field['id'], '" value="' . ( ! empty( $meta ) ? number_format( $meta, 8, '.', ',' ) : $field['std'] ) . '" /> ';
					} else {
						echo '<input class="cmb_text_money" type="text" name="', $field['id'], '" id="', $field['id'], '" value="' . ( ! empty( $meta ) ? number_format( $meta, 0, '.', ',' ) : $field['std'] ) . '" /> ';
					}
					break;
				case 'textarea':
	...
	function save( $post_id=null, $post=null ) {
		global $global_currency;
		if(empty($post_id) || $post_id==null) {
			if(!isset($_REQUEST['post_type']) || $_REQUEST['post_type'] != 'ignition_product' || !isset($_REQUEST['post_ID'])) {
				return;
			}

			// We'll only get this far if post_type is ignition_product
		...
		if ( ! empty( $_POST['ign_product_price'] ) ) {
			if ($global_currency == 'BTC' || $global_currency == 'credits') {
				$product_price = str_replace( ',', '', esc_attr( $_POST['ign_product_price'] ) );     //replacing commas with empty
			} else {
		...
		if ( $post->post_type == 'ignition_product' ) {
			...
			if ( ! empty( $project_id ) ) {
				if ($global_currency == 'BTC' || $global_currency == 'credits') {
					$product_goal = round( str_replace( ',', '', esc_attr( $_POST['ign_fund_goal'] ) ), 8 );      //replacing commas with empty
				} else {
				...
				$sql_update = $wpdb->prepare(
					'UPDATE ' . $wpdb->prefix . 'ign_products SET product_name = %s,
					ign_product_title = %s,
					ign_product_limit = %s,
					product_details = %s,
					product_price = %s,
				...
				if ($global_currency == 'BTC' || $global_currency == 'credits') {
					$product_goal = round( str_replace( ',', '', $_POST['ign_fund_goal'] ), 8 );        //replacing commas with empty
				} else {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			// Converting string date to unix timestamp while saving from "Edit post" screen
