...
function idc_orders_list($content) {
	...
	include IDC_PATH.'templates/_mdProfileTabs.php';
	$levels = ID_Member_Level::get_levels();
	include IDC_PATH.'templates/_orderList.php';
	if (isset($_GET['view_receipt'])) {
		$current_user = wp_get_current_user();
		$order_id = $_GET['view_receipt'];
		$order = new ID_Member_Order($order_id);

		// patch for stripe checout gateway
		if( isset($_GET['sr_checkout']) ) $order->activate_status();
		...
		if ($last_order->user_id == $current_user->ID) {
			...
			include 'templates/_orderLightbox.php';
		}
	}
	echo '</div>';
	$content = ob_get_contents();
...
function memberdeck_profile_form($content) {
	...
	if (!empty($idc_avatar)) {
		...
		if ( is_array( $idc_avatar_data ) ) { // Check if $avatar is an array
...
/**
* Filter user avatar when custom avatar is present
*/

function idc_filter_avatar($args, $id_or_email) {
	$usermeta = array();
	if (is_object($id_or_email)) {
		$object_type = get_class($id_or_email);
		switch ($object_type) {
			case 'WP_User':
				//
				break;
			case 'WP_Post':
				...
				break;
			case 'WP_Comment':
				$id_or_email = (!empty($object_type->comment_author_email) ? $object_type->comment_author_email : null);
				break;
		}
	}
	if (!empty($id_or_email) && is_string($id_or_email)) {
		// we have an email
	...
	if (is_int($id_or_email)) {
		...
		if (!empty($usermeta['idc_avatar'])) {
			$avatar = wp_get_attachment_image_src($usermeta['idc_avatar'][0], (isset($args['width']) && isset($args['height']) ? array($args['width'], $args['height']) : $args['size']));
			if ( is_array( $avatar ) ) { // Check if $avatar is an array
				$idc_avatar_url = $avatar[ 0 ];
				$args[ 'url' ] = $idc_avatar_url;
			}
		}
	}
	return $args;
...
add_filter('get_avatar_data', 'idc_filter_avatar', 10, 2);
...
add_filter('idc_dashboard_notification', 'idc_account_success');

function idc_account_success($notification) {
...
function idc_order_lightbox($notification) {
	...
	if (isset($_GET['idc_product']) && isset($_GET['paykey'])) {
		...
		if ( isset($last_order) ) {
			...
			//return $notification; Commented return statement so it keeps including the lightbox template - Issue #17
		}
...
function idc_upgrade_pathways_price($product_price, $product_id, $user_id, $ignore_upgrade) {
	...
	if ($ignore_upgrade === false) {
		...
		if ($level->level_type != 'recurring') {
			if (!class_exists('ID_Member_Pathways')) {
				require_once IDC_PATH.'classes/class-id-member-pathways.php';
			}
...
function idc_price_format($amount, $gateway = null) {
	...
	if ($gateway == 'BTC' || $gateway == 'coinbase') {
		$amount = number_format(preg_replace('/[^0-9.]+/', "", $amount), 8, '.', ',');
	}
...
function idc_fee_calculation($fee, $price, $fee_type, $gateway) {
	...
	if ($gateway == "pp-adaptive") {
		...
		if ($fee_type == 'percentage') {
			if ($global_currency == 'BTC' || $global_currency == 'credits') {
				$fee = number_format($price * ($fee / 100), 8, '.', '');
			} else {
...
function mdid_set_form($content) {
	...
	if (isset($_GET['level'])) {
		...
		if (!empty($owner)) {
			...
			ob_start();
			global $crowdfunding;
			include_once 'templates/_errorMsg.php';
			$content = ob_get_contents();
			ob_end_clean();
		}
	}
 ...
 */
function idc_currency_position($position, $post_id) {
	global $global_currency;
	if ($global_currency == "credits") {
		$position = "right";
	}
...
function idc_display_checkout_descriptions($content, $level, $level_price, $user_data, $gateways, $general, $credit_value) {
	global $global_currency;
	// Getting the required variables for the Description template
	$customer_id = customer_id();

	$type = isset($level->level_type)?$level->level_type:'lifetime';
	$recurring = isset($level->recurring_type)?$level->recurring_type:'none';
	$limit_term = isset($level->limit_term)?$level->limit_term:0;
	$term_length = isset($level->term_length)?$level->term_length:0;
	...
	if ($combined_product) {
		$gateway_array = array (
			'idc_free_checkout_description',
			'idc_paypal_checkout_description',
			'idc_credits_checkout_description',
			'idc_coinbase_checkout_description',
			'idc_offline_checkout_description'
		);
		foreach ($gateway_array as $gateway_description) {
			add_filter($gateway_description, function() {
				return null;
			});
		}
	...
	// Paypal currency
	$pp_currency = 'USD';
	// #devnote do we not have access to this info already?
	if (!empty($gateways)) {
		if (is_array($gateways)) {
			$pp_currency = (isset($gateways['pp_currency']) ? $gateways['pp_currency'] : 'USD');
		}
	}
	$cc_currency_symbol = apply_filters('idc_cc_desc_currency_sym', '$', $gateways);
	$cc_currency = apply_filters('idc_cc_desc_currency', 'USD', $gateways);
	// #devnote do we not have access to this info already?
	// Stripe currency
	...
	if (!empty($gateways)) {
		if (is_array($gateways)) {
			$stripe_currency = (isset($gateways['stripe_currency']) ? $gateways['stripe_currency'] : 'USD');
	...
	// #devnote do we not have access to this info already?
	// Coinbase currency
	// $ecb = (isset($gateways['ecb']) ? $gateways['ecb'] : '0'); hard-setting it to 0 for already activated installs
	$ecb = 0;
	if ($ecb) {
	...
	ob_start();
	include 'templates/_checkoutFreeDescription.php';
	...
	ob_start();
	include 'templates/_checkoutPayPalDescription.php';
	...
	ob_start();
	include 'templates/_checkoutCreditCardDescription.php';
	...
	ob_start();
	include 'templates/_checkoutCreditsDescription.php';
	...
	ob_start();
	include 'templates/_checkoutCoinbaseDescription.php';
	...
	ob_start();
	include 'templates/_checkoutOfflineDescription.php';
	...
	ob_start();
	include 'templates/_checkoutTrialDescription.php';
