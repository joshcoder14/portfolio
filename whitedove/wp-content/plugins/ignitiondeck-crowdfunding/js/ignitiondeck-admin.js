jQuery(document).ready(function() {
	...
	jQuery('body').on('change','#project_id_shortcodes',function () {
	...
	if (endTypeSetting == undefined) {
		jQuery('input[name="ign_end_type"]').eq(1).attr("checked","checked").prop("checked",true);
	}
		...
		jQuery.ajax({
			...
			success: function(res) {
				...
				if (json) {
					...
					jQuery('body').on('change','select[name="deck_select"]',function() {
						...
						if (selected > 0) {
								...
								success: function(res) {
									...
									if (res.length > 0) {
										...
										if (json) {
											jQuery.each(json, function(k,v) {
												if (v == 1) {
													jQuery('input[name="' + k + '"]').attr("checked","checked").prop("checked",true);
												}
												else {
							...
							jQuery('.deck-attr').removeAttr('checked').prop('checked', false);
	...
	if (jQuery('select[name="product_id"]').length > 0) {
		jQuery('body').on('change','select[name="product_id"]',function() {
			get_levels();
		});
		jQuery('body').on('change','#product_level',function() {
			change_price();
		});
		get_levels();
	}
	...
	jQuery('body').on('click','input[name="btn_generate_code"]',function(e) {
	...
	jQuery('body').on('click',".hide-notice",function(e) {
		e.preventDefault();
	...
	if (jQuery('#project-charge-screen').length > 0) {
		...
		jQuery('body').on('click','#btnProcessProjectPreauth',function(e) {
			e.preventDefault();
							...
							jQuery.ajax({
								...
								success: function(res) {
									...
									if (jQuery('#charge-notice').length > 0) {
										if (typeof (new_json.counts) === "undefined") {
											jQuery("#project-charge-confirm").html('<div id="charge-notice" class="updated fade below-h2" id="message"><p><span class="success-count">' + new_json.message + '</p><a id="close-notice" href="#">Close</a></div>');
										}
										else{
									...
									jQuery('#btnProcessProjectPreauth').removeAttr('disabled').prop('disabled', false);
								}
							});
						});
					}
    ...
    jQuery('.ignOrders .order-view .comments li a').on('click', function(e) {
        e.preventDefault();
        if(jQuery(this).html()=='Edit') {
            jQuery(this).parent().find('textarea').prop('disabled', false);
            jQuery(this).html('Update');
        } else {
            var order_id = jQuery(this).data('commentid');
            var comment = jQuery(this).parent().find('textarea').val();
            jQuery.ajax({
                url: id_ajaxurl,
                type: 'POST',
                data: {
                    action: 'idc_anon_checkout_comment_update',
                    order_id: order_id,
                    comment: comment
                },
				success: function(res) {
            ...
            jQuery(this).parent().find('textarea').prop('disabled', true);
