...
URI: https://www.ignitiondeck.com
...
function id_idf_check() {
	if ( ! class_exists( 'IDF' ) ) {
		...
		// we need to deactivate the ignitiondeck-crowdfunding plugin as well to avoid
		// fatal dependency errors.

		// For backend
		require_once ABSPATH . 'wp-admin/includes/plugin.php';
		deactivate_plugins( plugin_basename( __FILE__ ) );  // this deactivates IDCF

		if (function_exists('id_idc_check_2')) {
			// tell the IDC plugin to also deactivate itself
			id_idc_check_2(); // <--- this function is declared in the IDC plugin
		}

		wp_die( __( 'IgnitionDeck Crowdfunding requires installation of the IgnitionDeck Framework prior to activation.', 'ignitiondeck' )  . 
		"<br/> <a href='" . 
		...
		add_action( 'template_redirect', 'idf_frontend_error' );
		
	}
}

// This id_idf_check_2 function is called from the idcommerce plugin
// so that IDC can force the IDCF plugin to deactivate itself if IDF 
// isn't active.
function id_idf_check_2() {
	if ( ! class_exists( 'IDF' ) ) {
		// If the "IDF" class doesn't exist, then the ignitiondeck foundation 
		// plugin is either not installed, or not activated. In this case,
		// we need to deactivate the ignitiondeck-crowdfunding plugin as well to avoid
...
function ign_pre_install( $blog_id = null ) {
	...
	} elseif ( ! empty( $blog_id ) && is_id_pro() ) {
		if ( $blog_id == 1 ) {
			$prefix = $wpdb->prefix;
...
require_once 'classes/class-id_form.php';
require_once 'classes/class-project_widget.php';
require_once 'classes/class-id_project.php';
require_once 'classes/class-deck.php';
if ( is_id_pro() ) {
	include_once 'classes/class-id_fes.php';
	include_once 'classes/class-id_fes_team_info.php';
}
require_once 'classes/class-id_order.php';
require_once 'ignitiondeck-functions.php';
require_once 'ignitiondeck-cron.php';
require_once 'ignitiondeck-admin.php';
require_once 'ignitiondeck-postmeta.php';
require_once 'ignitiondeck-shortcodes.php';
//include_once 'ignitiondeck-globals.php';
if ( is_id_pro() ) {
	include_once 'ignitiondeck-enterprise.php';
}
//require_once 'ignitiondeck-update.php';
...
} elseif ( is_multisite() && is_id_network_activated() && file_exists( plugin_dir_path( dirname( __FILE__ ) ) . '/ignitiondeck/idf.php' ) ) {
	include_once plugin_dir_path( dirname( __FILE__ ) ) . '/ignitiondeck/idf.php';
}
if ( idf_exists() ) {
...
require_once 'ignitiondeck-api.php';
require_once 'ignitiondeck-filters.php';
...
function enqueue_front_js() {
	...
	wp_add_inline_script( 'ignitiondeck', 'var id_siteurl = "'. $id_siteurl . '";' );
}
...
add_action( 'id_set_defaults', 'disable_theme_restriction' );
function disable_theme_restriction () {
	update_option( 'idcf_auto_insert', 1 );
}
