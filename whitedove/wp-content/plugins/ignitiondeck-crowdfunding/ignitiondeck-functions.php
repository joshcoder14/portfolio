...
function is_id_project() {
	...
	if ( isset( $post ) ) {
		...
		} elseif ( strpos( $post_content, 'project_' ) ) {
	...
	if ( $project_id > 0 ) {
		...
		if ( ! empty( $post ) ) {
			...
			if ( get_option( 'permalink_structure' ) == '' ) {
				...
				} elseif ( get_post_meta( $post->ID, 'ign_option_project_url', true ) == 'page_or_post' ) {      // If Project URL is another post or Project page
				...
				} elseif ( get_post_meta( $post->ID, 'ign_option_project_url', true ) == 'external_url' && ! empty( $meta_url ) ) {     //If some external URL is set as Project page
					if ( $page == '' ) {
			...
			if ( ! empty( $post_page ) ) {
				...
				if ( $post_page !== 'default' ) {
					...
					if ( $permalink_structure == '' ) {
						...
						} elseif ( $post_page == 'page_or_post' ) {        // If Project URL is another post or Project page
						...
						} elseif ( $post_page == 'external_url' ) {        //If some external URL is set as Project page
...
function get_id_skins() {
	...
	$skins = array();
    if (isset($res->theme_choices) && !is_null($res->theme_choices) && is_serialized($res->theme_choices)) {
...
add_action( 'astra_template_parts_content_top', 'do_template_parts', 0 );
function do_template_parts() {
	if(isset($_GET['purchaseform'])) {
		...
		add_action( 'astra_entry_content_single', 'custom_entry_content_single_template' );
	}
}

function custom_entry_content_single_template() {
	global $post;
	$id = $post->ID;
	$post = get_post($id);
	...
	<div class="entry-content">
		<?php
		echo memberdeck_checkout(['product'=>$product_id]);
		?>
	</div>
	<?php
}
...
function override_single_template( $single_template ){
    global $post;
	if($post->post_type == 'ignition_product') {
		if( file_exists( get_stylesheet_directory() . '/single-ignition_product.php' ) ) {
			return $single_template;
		} elseif( file_exists( get_template_directory() . '/single-ignition_product.php' ) ) {
			return $single_template;
		} else {
			if(isset($_GET['purchaseform'])) {
				return dirname(__FILE__) .'/templates/single-ignition_product-form.php';
			} else {
				return dirname(__FILE__) .'/templates/single-ignition_product.php';
			}
		}
	}
...
function idc_anon_checkout_comment_update() {
	if ( isset( $_POST['order_id'] ) ) {
		$order_id = $_POST['order_id'];
		$comment = $_POST['comment'];
		$single = true;
		$meta_key = 'idc_checkout_comments';
		ID_Member_Order::update_order_meta($order_id, $meta_key, $comment, '', true);
		idf_cache_object('id_member_order-get_order_meta-'.$order_id.'-'.$meta_key.($single ? '-true' : '-false'), $meta, 86400);
	}
	exit;
}
add_action( 'wp_ajax_idc_anon_checkout_comment_update', 'idc_anon_checkout_comment_update' );
add_action( 'wp_ajax_nopriv_idc_anon_checkout_comment_update', 'idc_anon_checkout_comment_update' );
