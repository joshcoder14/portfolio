	...
	*/
	...
	function widget($args, $instance) {
		if (!empty($instance)) {
			...
			if ( $query->have_posts() ){
				echo '<div class="fh_widget fh_project_widget">';
				echo (!empty($instance['title']) ? '<h2 class="entry-title widget-title">'.$instance['title'].'</h3>' : '');
				echo '<div class="project-grid fh_widget '.(isset($instance['classes']) ? $instance['classes'] : '').'">';
	...
	function form($instance) {
		...
		$form .= '<select id="'.$this->get_field_id( 'orderby' ).'" name="'.$this->get_field_name( 'orderby' ).'">
					...
					<option value="date" '.(isset($orderby) && $orderby == 'date' ? 'selected="selected"' : '').'>'.__('Date', 'fivehundred').'</option>
				  </select>';
		$form .= '</label></p>';
		...
		$form .= '<select id="'.$this->get_field_id( 'order' ).'" name="'.$this->get_field_name( 'order' ).'">
					...
					<option value="DESC" '.(isset($order) && $order == 'DESC' ? 'selected="selected"' : '').'>Descending</option>
				  </select>';
		$form .= '</label></p>';
